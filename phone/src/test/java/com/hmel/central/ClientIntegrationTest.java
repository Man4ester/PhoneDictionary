...
public class ClientIntegrationTest {
	...
	try {
	    ...
	    assertThat(client.getLastName()).isEqualTo(client2.getLastName())
		    .as("The last name's should be equals");

	    for (int i = 0; i < client.getAdresses().size(); i++) {
		assertThat(client.getAdresses().get(i).getAddress()).isEqualTo(
			client2.getAdresses().get(i).getAddress()).as(
			"The address should be equals");
		assertThat(client.getAdresses().get(i).getCity()).isEqualTo(
			client2.getAdresses().get(i).getCity()).as(
			"The address should be equals");
	    }

	    for (int i = 0; i < client.getPhones().size(); i++) {
		assertThat(client.getPhones().get(i).getPhone()).isEqualTo(
			client2.getPhones().get(i).getPhone()).as(
			"The phones should be equals");
		assertThat(client.getPhones().get(i).getPhoneType()).isEqualTo(
			client2.getPhones().get(i).getPhoneType()).as(
			"The phones should be equals");
	    }
