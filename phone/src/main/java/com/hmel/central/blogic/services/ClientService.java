...
public class ClientService implements IClientService {
	...
	public Client saveUpdate(Client client) throws PhoneDictionaryException {
		...
		if (client == null) {
			throw new IllegalArgumentException(
					"Client can't be NULL for saving");
		}
	...
	@Override
	public List<Client> findByCriteria(DetachedCriteria criteria,
			int firstResult, int maxResult) throws PhoneDictionaryException {
		if (criteria == null) {
		...
		List<Client> res = new ArrayList<Client>();
		try {
			res = (List<Client>) criteria.getExecutableCriteria(getSession())
